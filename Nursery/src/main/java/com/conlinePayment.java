...
public class conlinePayment extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String qur3="select *from plant_book where cust_id='"+uid+"' and item_quant='"+cust_qunt+"' and item_id='"+fid+"' and payment_status='done'";
		...
		String qur="insert into plant_book(custname,cust_id,plant_cat,item_name,item_id,item_quant,total_cost,address,payment_type) values('"+username+"','"+uid+"','"+catagory+"','"+pic+"','"+fid+"','"+cust_qunt+"','"+totalcost+"','"+address+"','"+pay_type+"')";
		...
		{
			response.sendRedirect("bambooPlants.jsp?book=success");	
		}
		
		
		else if(plantname.equals("cactus")){
			response.sendRedirect("cactusPlants.jsp?book=success");		
		}
	
		else if(plantname.equals("Flowering Creepers")){
			response.sendRedirect("flowersPlants.jsp?book=success");		
		}
