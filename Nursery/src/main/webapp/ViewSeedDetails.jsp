	...
	<div class="container" style="margin-top: 70px;">
		...
		<%
					...
					<h5>Item Price ---> <%=price %></h5>	<br>
												...
												<button type="submit" style="margin: 3px" onclick="location.href='addCart?username=<%=username%>&itemtype=Seed&type=<%=type%>&pic=<%= imagename %>&price=<%=price%>&item_qnt=<%= item_qnt %>&item_name=<%=item_name%>&cat_plant=<%=rs1.getString("item_type")%>&fid=<%=id%>'">Add Cart</button>
						
<%-- 						
						
						<button type="submit" style="margin: 3px" onclick="location.href='seedPaymant.jsp?type=<%=type%>&pic=<%= imagename %>&price=<%=price%>&item_qnt=<%= item_qnt %>&item_name=<%=item_name%>&cat_plant=<%=rs1.getString("item_type")%>&fid=<%=id%>'">Add Cart</button>
 --%>						</div>
